...
; NOTE: EVERY FUNCTION WORKS IN THE INTERACTIONS MENU, BUT CHECK-EXPECT
; FUNCTIONALITY BROKEN (MORE EXPLAINED DOWN BELOW)
...
;   pairing up a game object, such as a guessed letter, with
...
(define (read-dictionary str)
  (cond [(file-exists? str)
         (map (λ (s) (string-upcase s)) (read-lines str))]
        [else '()]))



; write-file : String String –> String
; Takes the file name and contents and writes and returns the file's name
...
(define (score str1 str2)
  (local [(define EXPLODED-STR1 (explode str1))
          (define EXPLODED-STR2 (explode str2))
          ; check-status : 1String 1String –> Pair
          ; Returns the pair of the letter with it's associated status
          (define (check-status s1 s2)
            (cond [(string=? s1 s2)
                   (make-pair s1 LS-RIGHT)]
                  [(string-in-list? s1 EXPLODED-STR2)
                   (make-pair s1 LS-MISPLACED)]
                  [else (make-pair s1 LS-WRONG)]))]
    (mymap2 EXPLODED-STR1 EXPLODED-STR2 check-status)))
...
(define (partial-guess->full-guess s len)
  (local [; add-spacers : String Nat –> String
          ; Creates a padding of spaces
          (define (add-spacers s len)
            (replicate (- len (string-length s)) " "))]
    (string-append (implode (build-list (string-length s)
                                        (λ (n) (list-ref (explode s) n)))) (add-spacers s len))))
...
(define WS-WORD4 (make-wordle (list "CAT" "DAB") "BAD"))
...
(define (key-wordle ws ke los num)
  (cond [(and (string=? ke "\r") (= (string-length (wordle-current ws)) num))
         (if (ormap (λ (w) (string=? (wordle-current ws) w)) los)
             (make-wordle (cons (wordle-current ws) (wordle-past ws)) "")
             (make-wordle (wordle-past ws) (wordle-current ws)))]
             
        [(string=? ke "\r") ws]
        [(and (string=? ke "\b") (not (string=? "" (wordle-current ws))))
         (make-wordle (wordle-past ws)
                      (substring (wordle-current ws) 0 (- (string-length (wordle-current ws)) 1)))]
        [(string=? ke "\b")
         (make-wordle (wordle-past ws) (wordle-current ws))]
        [(and (string-alphabetic? ke) (not (= (string-length (wordle-current ws)) num)))
...
(define (end-wordle? ws s num)
  (cond [(string=? (wordle-current ws) s) #true]
        [(>= (length (wordle-past ws)) num) #true]
        [(cons? (wordle-past ws)) (ormap (λ (str) (string=? str s)) (wordle-past ws))]
        [else #false]))
...
(check-expect (draw-end-wordle WS-START "BAD" 5)
              (text "You lose\n\n:(\n\nBetter luck\nnext time!" 30 "red"))
(check-expect (draw-end-wordle WS-TYPE3 "CAT" 1)
              (text
               (string-append
                "Excellent job!\nYou win!\n\n:)\n\nYou guessed\nthe word\non your\nfirst try!")
               30 "darkgreen"))
(check-expect (draw-end-wordle WS-WORD4 "BAD" 3)
              (text
               (string-append "Good job!\nYou win!\n\n:)\n\nYou guessed\nthe word\nin 3 tries!")
               30 "darkgreen"))



(define (draw-end-wordle ws s num)
  (cond [(string=? (wordle-current ws) s)
         (if (= (add1 (length (wordle-past ws))) 1)
             (text
              (string-append
               "Excellent job!\nYou win!\n\n:)\n\nYou guessed\nthe word\non your\nfirst try!")
              30 "darkgreen")
             (text (string-append "Good job!\nYou win!\n\n:)\n\nYou guessed\nthe word\nin "
                                  (number->string (add1 (length (wordle-past ws)))) " tries!")
                   30 "darkgreen"))]
        [else (text "You lose\n\n:(\n\nBetter luck\nnext time!" 30 "red")]))
...
(check-expect (draw-wordle WS-START "BAD" 4)
              (stack/v (list
                        (draw-current WS-START "BAD")
                        (stack/v (draw-future WS-START "BAD" 4)))))
(check-expect (draw-wordle WS-WORD3 "BAD" 3)
              (stack/v (list (stack/v (reverse (draw-past WS-WORD3 "BAD")))
                             (draw-current WS-WORD3 "BAD"))))

(define (draw-wordle ws s num)
  (cond [(empty? (wordle-past ws)) (stack/v (list
                                             (draw-current ws s)
                                             (stack/v (draw-future WS-START s num))))]
        [(zero? (- (sub1 num) (length (draw-past ws s))))
         (stack/v (list (stack/v (reverse (draw-past ws s)))
                        (draw-current ws s)))]
        [(empty? (draw-future WS-START s (- num (length (draw-past ws s)))))
         (stack/v (list (stack/v (reverse (draw-past ws s)))
                        (draw-current ws s)))]
        [else (stack/v (list (stack/v (reverse (draw-past ws s)))
                             (draw-current ws s)
                             (stack/v (draw-future WS-START s (- num (length (draw-past ws s)))))))]))


; draw-current : WS String -> Image
; Draws one row of the current state of the game

(check-expect (draw-current WS-START "BAD") (stack/h
                                             (list
                                              (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR)))))
(check-expect (draw-current WS-TYPE1 "BAD") (stack/h
                                             (list
                                              (overlay (text "C" (/ LT-SIZE 2) BG-COLOR)
                                                       (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR)))))
(check-expect (draw-current WS-TYPE3 "BAD") (stack/h
                                             (list
                                              (overlay (text "C" (/ LT-SIZE 2) BG-COLOR)
                                                       (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (text "A" (/ LT-SIZE 2) BG-COLOR)
                                                       (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR))
                                              (overlay (text "T" (/ LT-SIZE 2) BG-COLOR)
                                                       (square LT-SIZE "outline" BORDER-COLOR)
                                                       (square LT-SIZE "solid" GUESS-COLOR)))))

(define (draw-current ws s)
  (partial-guess->image (wordle-current ws) (string-length s)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; NOTE: FROM HERE ON OUT, SOME CHECK-EXPECTS COMMENTED OUT – stack/v KEPT RETURNING A
; LIST OF IMAGES WHEN IT ISN'T SUPPOSED TO IN THE TEST BUT WOULD RETURN A SINGLE IMAGE
; WHEN PLUGGED DIRECTLY INTO THE INTERACTIONS WINDOW (CHECK-EXPECT FUNCTION IS BROKEN)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; draw-past : WS String –> [List-of Image]
; Draws all previous rows of guesses

(check-expect (draw-past WS-WORD1 "BAD") (list
                                          (row->image
                                           (list (make-pair (make-pair "C" LS-WRONG) lsp->image)
                                                 (make-pair (make-pair "A" LS-RIGHT) lsp->image)
                                                 (make-pair (make-pair "T" LS-WRONG) lsp->image)))))

(check-expect (draw-past WS-TYPE3 "BAD") '())
(check-expect (draw-past WS-START "BAD") '())

(check-expect (draw-past WS-WORD3 "BAD") (list
                                          (row->image
                                           (list (make-pair (make-pair "C" LS-WRONG) lsp->image)
                                                 (make-pair (make-pair "A" LS-RIGHT) lsp->image)
                                                 (make-pair (make-pair "T" LS-WRONG) lsp->image)))
                                          (row->image
                                           (list (make-pair (make-pair "D" LS-MISPLACED) lsp->image)
                                                 (make-pair (make-pair "A" LS-RIGHT) lsp->image)
                                                 (make-pair (make-pair "B" LS-MISPLACED) lsp->image)))
                                          (row->image
                                           (list (make-pair (make-pair "B" LS-RIGHT) lsp->image)
                                                 (make-pair (make-pair "A" LS-RIGHT) lsp->image)
                                                 (make-pair (make-pair "D" LS-RIGHT) lsp->image)))))

(define (draw-past ws s)
  (map (λ (wp) (row->image (map (λ (lsp) (make-pair lsp lsp->image)) (score wp s))))
       (wordle-past ws)))

; draw-future : WS String Nat –> [List-of Image]
; Draws all future rows of guesses

(check-expect (draw-future WS-START "BAD" 4) (list
                                              (stack/h
                                               (list
                                                (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                         (square LT-SIZE "solid" GUESS-COLOR))
                                              ...
                                              (stack/h
                                               (list
                                                ...
                                                (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                         (square LT-SIZE "solid" GUESS-COLOR))))))
(check-expect (draw-future WS-WORD1 "BAD" 4) (list
                                              (stack/h
                                               (list
                                                (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                         (square LT-SIZE "solid" GUESS-COLOR))
                                              ...
                                              (stack/h
                                               (list
                                                ...
                                                (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                         (square LT-SIZE "solid" GUESS-COLOR))))))
(check-expect (draw-future WS-WORD3 "BAD" 5) (list
                                              (stack/h
                                               (list
                                                (overlay (square LT-SIZE "outline" BORDER-COLOR)
                                                         (square LT-SIZE "solid" GUESS-COLOR))
...
(define (draw-future ws s num)
  (cond [(or (zero? num) (= num 1)) '()]
        [else (cons (partial-guess->image "" (string-length s)) (draw-future ws s (sub1 num)))]))
